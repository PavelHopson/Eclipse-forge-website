...
Текущее направление Landing — `Editorial Eclipse Cinema`: один доминирующий диск затмения,
oversized wordmark, сдержанная солнечная корона и почти неподвижная глубина. От Noema/AERRA/
Lumen-подобных референсов берутся только общие композиционные принципы; внешний код, шрифты,
модели и bucket-ассеты не копируются. Hero обязан проходить 3-second test без анимации.
